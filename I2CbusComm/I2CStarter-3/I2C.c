...
uint8_t I2C0_Receive(uint8_t slave_addr, uint8_t slave_reg_addr){
	...
	I2C0_MSA_R = (slave_addr<<1)&0xFE;								// Slave Address is the 7 MSB
...
uint8_t I2C0_Transmit(uint8_t slave_addr, uint8_t slave_reg_addr, uint8_t data){
	...
	I2C0_MSA_R = (slave_addr<<1)&0xFE;								//Slave Address is the first 7 MSB
	I2C0_MSA_R &= ~I2C0_RW_PIN; 						// Clear LSB to write
	I2C0_MDR_R = slave_reg_addr&0xFF;								//Transmit register addr to interact
...
uint8_t I2C0_Burst_Transmit(uint8_t slave_addr, uint8_t slave_reg_addr, uint8_t* data, uint32_t size){
	...
	while(size > 1){
		...
		I2C0_MDR_R = (*data)&0xFF;							//Deference Pointer from data array and load into data reg. Post-Increment the pointer after
	...
	I2C0_MDR_R = (*data)&0xFF;								//Deference Pointer from data array and load into data reg
